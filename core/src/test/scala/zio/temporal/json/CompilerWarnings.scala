...
/** Drives the Scala 3 compiler in-process to capture every diagnostic — including warnings — emitted while
  * compiling a code snippet against this test run's actual classpath.
  *
  * `scala.compiletime.testing.typeCheckErrors` can't be used for this: it's a compiler intrinsic that only
  * ever surfaces errors, silently dropping warnings before returning (verified empirically — feeding it a
  * snippet that demonstrably warns under a normal `sbt compile` still returns `Nil`).
  */
object CompilerWarnings {
  ...
  /** This test run's actual classpath, so the driven compiler resolves `zio.temporal.*` / `zio.json.*` etc.
    * exactly as the enclosing sbt test run does. sbt doesn't fork tests in this build, so the context
    * classloader IS the classloader sbt built for this test run — walk it for its URLs.
    */
  ...
  /** Compiles `source` as a standalone `.scala` file against this test run's classpath and returns every
    * diagnostic (errors, warnings, info) the compiler emitted for it.
    */
